<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            @lang('crud.evaluation_results.index_title')
        </h2>
    </x-slot>
                ...
                <div class="block w-full overflow-auto scrolling-touch">
                    <table class="w-full max-w-full mb-4 bg-transparent">
                        ...
                        <tbody class="text-gray-600">
                            @forelse($evaluationResults as $evaluationResult)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 text-left">
                                    {{ $evaluationResult->mark ?? '-' }}
                                </td>
                                <td class="px-4 py-3 text-left">
                                    {{
                                ...
                                >
                                    <div
                                        ...
                                        "
                                    >
                                        @can('update', $evaluationResult)
                                        <a
                                            href="{{ route('evaluation-results.edit', $evaluationResult) }}"
                                            class="mr-1"
                                        >
                                            <button
                                                type="button"
                                        ...
                                        @endcan @can('view', $evaluationResult)
                                        <a
                                            href="{{ route('evaluation-results.show', $evaluationResult) }}"
                                            class="mr-1"
                                        >
                                            <button
                                                type="button"
                                        ...
                                        $evaluationResult)
                                        <form
                                            action="{{ route('evaluation-results.destroy', $evaluationResult) }}"
                                            method="POST"
